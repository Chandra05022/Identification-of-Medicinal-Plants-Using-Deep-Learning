...
mydb = mysql.connector.connect(
    host="localhost", user="root", password="", port="3306", database="medical_plant"
)
...
class MobileNetRNN(nn.Module):
    def __init__(self, num_classes):
        ...
        self.rnn = RNNClassifier(
            self.rnn_input_dim, self.rnn_hidden_dim, self.rnn_output_dim
        )
...
class_names = ['Aloevera', 'Amla', 'Amruta_Balli', 'Arali', 'Ashoka', 'Ashwagandha', 'Avacado',
...
def predict_classification(image_path):
    ...
    if plant_name == "Wood sorrel":
        description = (
            "Wood sorrel is a wild herb with a sour taste, often used in salads."
        )
        ...
        uses = (
            "Used in traditional remedies to improve health and treat various ailments."
        )
        ...
        uses = (
            "Used to produce castor oil for industrial applications and as a laxative."
        )
    ...
    elif plant_name == "Betel Nut":
        description = "Betel nut is the seed of the Areca palm, used in chewing and for medicinal purposes."
        uses = (
            "Used in traditional chewing practices and in some medicinal applications."
        )
    elif plant_name == "Neem":
        description = (
            "Neem is a tree known for its antimicrobial and medicinal properties."
        )
        uses = "Used in traditional medicine and personal care products for its health benefits."
    elif plant_name == "Jasmine":
        description = (
            "Jasmine is a fragrant flower used in perfumes and traditional medicine."
        )
        ...
        description = (
            "Mint is a fragrant herb used in cooking, teas, and for medicinal purposes."
        )
        ...
        description = (
            "Pomegranate is a fruit known for its health benefits and rich flavor."
        )
        ...
        description = (
            "Geranium is a plant known for its aromatic leaves and ornamental value."
        )
        ...
        description = (
            "Amla is a fruit known for its high vitamin C content and health benefits."
        )
        uses = (
            "Used in traditional medicine to boost immunity and in health supplements."
        )
    elif plant_name == "Ekka":
        description = (
            "Ekka is a plant used in traditional medicine with various health benefits."
        )
        ...
        uses = (
            "Used to reduce stress, improve energy levels, and support overall health."
        )
        ...
        description = (
            "Avocado is a fruit known for its creamy texture and nutritional benefits."
        )
        ...
        uses = (
            "Consumed fresh or used in smoothies and salads for its digestive benefits."
        )
    ...
    if request.method == "POST":
        ...
        if password != c_password:
            return render_template(
                "register.html", message="Confirm password does not match!"
            )

        if len(mobile_number) != 10 or not mobile_number.isdigit():
            return render_template(
                "register.html", message="Mobile number must be exactly 10 digits!"
            )
        ...
        if email_data:
            return render_template(
                "register.html", message="This email ID already exists!"
            )
        ...
        try:
            ...
            return render_template(
                "register.html", message="Database error: " + str(err)
            )
...
def upload():
    if request.method == "POST":
        ...
        fn = myfile.filename
        if myfile and myfile.filename.split(".")[-1].lower() in [
            "jpg",
            "png",
            "jpeg",
            "jfif",
            ...
            if relevance == 0:  # If relevant
                ...
                string=statename.stateMyPoint(classification)
                ...
                return render_template(
                    "upload.html",
                    mypath=mypath,
                    prediction="This image is not relevant to this project!",
                    fn=fn,
                )
